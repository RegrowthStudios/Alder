        ...
        public function assert(Acl $acl, RoleInterface $role = NULL, ResourceInterface $resource = NULL, $privilege = NULL) {
            ...
            if (($usageCount = $visitorData["usage_counts"][$ruleUID]) < $frequencySettings["max_count"]) {
                ...
                if (($cooldownCount = $visitorData["cooldown_counts"][$ruleUID]) > 0) {
                    ...
                    $cooldownExpire = $visitorData["last_cooldown_start_times"][$ruleUID] + $frequencySettings["cooldown_period"] * pow($frequencySettings["cooldown_coefficient"], $cooldownCount);
                    if ($cooldownExpire > time()) {
                        // TODO(Matthew): Update visitor data for new usage count and last usage time.
                        return false;
                    }
                    if ($visitorData["last_cooldown_start_times"][$ruleUID] + $frequencySettings["cooldown_reset_period"] <= time()) {
