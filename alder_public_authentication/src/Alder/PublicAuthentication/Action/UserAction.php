    ...
    class UserAction extends AbstractRestfulAction
    {
        ...
        protected function create($data) {
            ...
            if (!is_array($data)) {
                $this->response = new JsonResponse([
                    "errors" => [
                        101030201 => $errorContainer->retrieveErrorString(101030201)
                    ]
                ], 400);
                return;
            }
            ...
            if (!is_array($data[0])) {
                ...
                foreach ($data as $datum) {
                    if (!is_array($datum)) {
                        $this->response = new JsonResponse([
                            "errors" => $errorContainer->retrieveErrorString(101030203)
                        ], 400);
                        return;
                    }
                }
        ...
        public function replace($data) {
            ...
            if (!is_array($data)) {
                $this->response = new JsonResponse([
                    "errors" => [
                        101030401 => $errorContainer->retrieveErrorString(101030401)
                    ]
                ], 400);
                return;
            }
            ...
            if (!is_array($data[0])) {
                ...
                foreach ($data as $datum) {
                    if (!is_array($datum)) {
                        $this->response = new JsonResponse([
                            "errors" => $errorContainer->retrieveErrorString(101030403)
                        ], 400);
                        return;
                    }
                }
        ...
        protected function createUserFromData(array $data, UserTable& $userTable, ErrorContainer& $errorContainer) {
            ...
            \*******************************************************************/
            $user->employee_flag = (isset($data["employee_flag"]) && $data["employee_flag"] instanceof bool) ? $data["employee_flag"] : false;
